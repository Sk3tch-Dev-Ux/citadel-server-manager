/**
 * HTTP client for the citadel-hub.com Cloud Bans API.
 *
 * Mirrors backend/lib/license/client.js patterns — same lightweight
 * Node http/https with no axios/got dependency.
 *
 * Base URL: CITADEL_LICENSE_API or default https://api.citadel-hub.com.
 * The Fastify API lives on the `api.` subdomain; the marketing Next.js site is
 * at the apex `citadel-hub.com` and does NOT serve /api routes — so the default
