...
async function validateSteamLogin(username, password, guardCode) {
  ...
  addLog('info', 'steam', `Validating Steam login for: ${username}${guardCode ? ' (with guard code)' : ''}...`);
  ...
  return new Promise((resolve) => {
    ...
    const done = (result) => {
      ...
      clearTimeout(timeout);
      try { proc.kill(); } catch {}
      addLog('info', 'steam', `Login validation result: ${JSON.stringify(result)}`);
      resolve(result);
    };
    ...
    const handleData = (data) => {
      const text = data.toString();
      output += text;
      addLog('info', 'steam', `SteamCMD: ${text.trim()}`);

      // Detect Steam Guard prompt in real-time — kill immediately, don't wait
      if (text.includes('Steam Guard') || text.includes('Two-factor') ||
          text.includes('two factor') || text.includes('Enter the current code') ||
          text.includes('Two-Factor') || text.includes('authenticator')) {
      ...
      // Detect login success
        ...
        // Don't resolve yet — let +quit finish cleanly — but mark success
        // Actually resolve now since we have confirmation
        done({ success: true });
      }
      // Detect bad credentials
      else if (text.includes('Invalid Password') || text.includes('Login Failure') || text.includes('FAILED login')) {
        done({ success: false, error: 'Invalid username or password' });
      }
      // Detect rate limiting
      else if (text.includes('rate limit') || text.includes('too many')) {
        done({ success: false, error: 'Too many login attempts — wait a few minutes and try again' });
      }
    };
    ...
    // Timeout: 30 seconds should be enough for a login test
    const timeout = setTimeout(() => {
      // If we got "Waiting for user info...OK" anywhere, that's success even if +quit didn't work
      if (output.includes('Waiting for user info...OK') || output.includes('Logged in OK')) {
        done({ success: true });
      } else {
        done({ success: false, error: 'Login timed out — SteamCMD may be updating. Try again.' });
      }
    }, 30000);

    proc.on('exit', () => {
      if (resolved) return;
      // Process exited without us catching a clear result
      if (output.includes('Logged in OK') || output.includes('Waiting for user info...OK')) {
        done({ success: true });
      } else if (output.includes('Steam Guard') || output.includes('Two-factor') || output.includes('Enter the current code')) {
        ...
        done({ success: false, error: `Login failed: ${output.slice(-200)}` });
      } else {
        // SteamCMD exited cleanly — probably worked
        done({ success: true });
      }
    });
