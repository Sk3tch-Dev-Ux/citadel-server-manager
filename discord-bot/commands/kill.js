...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const remaining = checkCooldown(interaction.user.id, 'kill');
    if (remaining > 0) {
    ...
    const result = await panelAction('actionKill', { steamId }, interaction.guildId, interaction);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    setCooldown(interaction.user.id, 'kill');
