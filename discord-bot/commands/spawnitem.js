...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt => opt.setName('item').setDescription('Item class name (e.g. Hatchet, AKM)').setRequired(true))
    .addIntegerOption(opt => opt.setName('quantity').setDescription('Quantity (default: 1, max: 100)').setRequired(false)),
  ...
  async execute(interaction) {
    ...
    const remaining = checkCooldown(interaction.user.id, 'spawnitem');
    if (remaining > 0) {
    ...
    const quantity = Math.min(Math.max(interaction.options.getInteger('quantity') || 1, 1), 100);

    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const result = await panelAction('actionSpawnItem', { steamId, itemClass, quantity }, interaction.guildId, interaction);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    setCooldown(interaction.user.id, 'spawnitem');
