...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const remaining = checkCooldown(interaction.user.id, 'heal');
    if (remaining > 0) {
    ...
    const result = await panelAction('actionHeal', { steamId }, interaction.guildId, interaction);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    setCooldown(interaction.user.id, 'heal');
