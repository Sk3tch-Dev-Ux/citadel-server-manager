...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('unstuck')
    .setDescription('Unstuck a player (teleport to surface at current position)')
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    const remaining = checkCooldown(interaction.user.id, 'unstuck');
    if (remaining > 0) {
    ...
    const result = await panelAction('actionUnstuck', { steamId }, interaction.guildId, interaction);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    setCooldown(interaction.user.id, 'unstuck', 'admin');
