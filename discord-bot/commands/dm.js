...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('dm')
    .setDescription('Send a private message to a player in-game (Admin Action)')
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    if (!message || !message.trim()) {
      return await interaction.reply({
        embeds: [buildErrorEmbed('Invalid Input', 'Message cannot be empty.')],
        flags: MessageFlags.Ephemeral,
      });
    }

    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const result = await panelAction('actionMessage', { steamId, message: message.trim() }, interaction.guildId, interaction);
