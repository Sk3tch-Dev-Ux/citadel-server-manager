...
- **GitHub repo rename — DONE** (WS1b, PRs #18–21, 2026-06): repo is now `Sk3tch-Dev-Ux/citadel-server-manager`; every hardcoded feed ref re-pointed (`agent-updater.js:31`, `desktop/src/auto-updater.js:49`, `desktop/package.json:57`, `installer/build.js`, cloud `config.ts` `GITHUB_REPO` default + `.env*.example` + Coolify `GITHUB_REPO`); URL tests updated; stale `…/DayzServerController` links in README/docs/cloud-web swept to the new name (2026-06-17). GitHub's rename redirect remains as a transition cushion.
- **Domain consolidation to citadel-hub.com — code DONE** (2026-06-17): canonical scheme is `citadel-hub.com` (marketing) + `app.citadel-hub.com` (app) + `api.citadel-hub.com` (API). Both repos swept; agent API/WS defaults + auto-update allowlist (dual-host, transition-safe) + mod endpoint + cloud CSP/SEO/env/Traefik updated; JWT issuer + email senders deliberately stay `citadels.cc`. Remaining = the runtime cutover (DNS/Traefik/env + a release) per `WS1B_REBRAND_RUNBOOK.md`.
